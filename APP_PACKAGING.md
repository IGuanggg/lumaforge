...
也可以在 GitHub Actions 手动运行 `Build macOS Release` workflow；推送 `v2.0.21` tag 时，该 workflow 会在 macOS runner 上构建并把 macOS zip 上传到 GitHub Release。
...
xcrun notarytool submit "releases/LumaForge-2.0.21-macos.zip" --keychain-profile "notarytool-profile" --wait
```
