...
- 智能画布：输出节点等待/失败/空结果状态明确显示；图片选中浮动工具条（高清、去背景、画笔、多角度、画同款、下载）；右键菜单补齐复制、粘贴、创建副本、发送至对话、保存到资产库、翻转、锁定等管理操作。
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -f Dockerfile.cloud \
  -t iguang9881/lumaforge-cloud:2.0.5 \
...
docker pull iguang9881/lumaforge-cloud:2.0.5
...
docker run -d \
  ...
  -e CLOUD_APP_VERSION=2.0.5 \
  -p 127.0.0.1:8787:8787 \
  -v /opt/lumaforge-cloud/cloud-data:/app/data \
  iguang9881/lumaforge-cloud:2.0.5
```
...
```powershell
.\scripts\check_release.ps1 -Version 2.0.5
```
