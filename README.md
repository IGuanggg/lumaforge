...
v2.0.24 是智能画布稳定修复版：在 v2.0.20 基础上修复多图生成部分成功、任务 loading 兜底、鼠标操作、连线删除和输入框粘贴体验，重点保证智能画布主链路稳定，不新增高风险功能。
...
也可以在 GitHub Actions 里手动运行 `Build macOS Release` workflow，或推送 `v2.0.24` tag 后让 macOS runner 自动构建并上传 macOS 包。
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -f Dockerfile.cloud \
  -t iguang9881/lumaforge-cloud:2.0.24 \
...
```powershell
.\scripts\check_release.ps1 -Version 2.0.24
```

GitHub Release 建议同时上传：

- `releases/LumaForge-Setup-2.0.24.exe` 安装器
- `releases/LumaForge-2.0.24-desktop.zip` 桌面自动更新包
- `releases/LumaForge-2.0.24-macos.zip` macOS 包（在 macOS 上构建）
- 对应 SHA256 校验信息
