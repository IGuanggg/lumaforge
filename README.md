...
v20.0.23 是智能画布稳定修复版：在 v2.0.20 基础上修复多图生成部分成功、任务 loading 兜底、鼠标操作、连线删除和输入框粘贴体验，重点保证智能画布主链路稳定，不新增高风险功能。
...
也可以在 GitHub Actions 里手动运行 `Build macOS Release` workflow，或推送 `v20.0.23` tag 后让 macOS runner 自动构建并上传 macOS 包。

输出：

```text
releases/LumaForge-20.0.23-macos.zip
releases/LumaForge-20.0.23-macos.sha256.txt
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -f Dockerfile.cloud \
  -t iguang9881/lumaforge-cloud:20.0.23 \
...
```powershell
.\scripts\check_release.ps1 -Version 20.0.23
```

GitHub Release 建议同时上传：

- `releases/LumaForge-Setup-20.0.23.exe` 安装器
- `releases/LumaForge-20.0.23-desktop.zip` 桌面自动更新包
- `releases/LumaForge-20.0.23-macos.zip` macOS 包（在 macOS 上构建）
- 对应 SHA256 校验信息
