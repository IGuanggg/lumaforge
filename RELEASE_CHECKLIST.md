...
- Confirm 2.0.2 cache-busted entries load the current GPT 对话, 细节增强, 无限画布, and 素材库 pages.
...
```powershell
.\build_desktop.bat
Compress-Archive -Path "dist\LumaForge\*" -DestinationPath releases\LumaForge-desktop-v2.0.2-windows.zip -Force
```

Cloud Docker:

```bash
docker buildx build --platform linux/amd64,linux/arm64 -f Dockerfile.cloud -t iguang9881/lumaforge-cloud:2.0.2 -t iguang9881/lumaforge-cloud:latest --push .
```
