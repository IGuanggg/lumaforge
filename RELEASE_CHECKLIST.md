...
- Confirm 2.0.12 cache-busted entries load the current GPT 对话, 细节增强, 无限画布, 素材库, and 应用设置 pages.
...
- GitHub Release must include `releases\LumaForge-2.0.12-desktop.zip`, not only a single EXE.
- If Inno Setup is installed, confirm `releases\LumaForge-Setup-2.0.12.exe` exists.
...
```bash
docker buildx build --platform linux/amd64,linux/arm64 -f Dockerfile.cloud -t iguang9881/lumaforge-cloud:2.0.12 -t iguang9881/lumaforge-cloud:latest --push .
```
