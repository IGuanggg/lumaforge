...
- Confirm 2.0.11 cache-busted entries load the current GPT 对话, 细节增强, 无限画布, 素材库, and 应用设置 pages.
...
- GitHub Release must include `releases\LumaForge-2.0.11-desktop.zip`, not only a single EXE.
- If Inno Setup is installed, confirm `releases\LumaForge-Setup-2.0.11.exe` exists.
...
```bash
docker buildx build --platform linux/amd64,linux/arm64 -f Dockerfile.cloud -t iguang9881/lumaforge-cloud:2.0.11 -t iguang9881/lumaforge-cloud:latest --push .
```
...
docker pull iguang9881/lumaforge-cloud:2.0.11
...
docker run -d \
  ...
  -e CLOUD_APP_VERSION=2.0.11 \
  -p 127.0.0.1:8787:8787 \
  -v /opt/lumaforge-cloud/cloud-data:/app/data \
  iguang9881/lumaforge-cloud:2.0.11
```
