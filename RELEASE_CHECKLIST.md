...
- Confirm 20.0.23 cache-busted entries load the current GPT 对话, 细节增强, 无限画布, 素材库, and 应用设置 pages.
...
- GitHub Release must include `releases\LumaForge-20.0.23-desktop.zip`, not only a single EXE.
- If Inno Setup is installed, confirm `releases\LumaForge-Setup-20.0.23.exe` exists.
...
```bash
VERSION=20.0.23 bash scripts/build_macos_release.sh
```

- macOS must be built on macOS; Windows cannot produce a real `.app`.
- GitHub Release should include `releases/LumaForge-20.0.23-macos.zip` when built on a Mac or macOS CI runner.
- For public distribution, sign and notarize with Apple Developer `codesign` and `xcrun notarytool`.

Cloud Docker:

```bash
docker buildx build --platform linux/amd64,linux/arm64 -f Dockerfile.cloud -t iguang9881/lumaforge-cloud:20.0.23 -t iguang9881/lumaforge-cloud:latest --push .
```
...
docker pull iguang9881/lumaforge-cloud:20.0.23
...
docker run -d \
  ...
  -e CLOUD_APP_VERSION=20.0.23 \
  -p 127.0.0.1:8787:8787 \
  -v /opt/lumaforge-cloud/cloud-data:/app/data \
  iguang9881/lumaforge-cloud:20.0.23
```
