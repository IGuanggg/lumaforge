...
func TestLumaUpdateCheckSelectsNewerDesktopRelease(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode([]map[string]any{
			{
				"tag_name":   "v2.1.15",
				"name":       "LumaForge 2.1.15",
				"draft":      false,
				"prerelease": false,
				"body":       "quality release",
				"assets": []map[string]any{
					{"name": "LumaForge-2.1.15-web.zip", "browser_download_url": "https://cdn.example.com/web.zip", "size": 10},
					{"name": "LumaForge-2.1.15-desktop.zip", "browser_download_url": "https://cdn.example.com/desktop.zip", "size": 20},
				},
			},
			{
	...
	if result["ok"] != true || result["configured"] != true || result["latest_version"] != "2.1.15" || result["is_newer"] != true {
	...
	if asset["name"] != "LumaForge-2.1.15-desktop.zip" || asset["url"] != "https://cdn.example.com/desktop.zip" {
		t.Fatalf("selected asset = %#v, want desktop zip", asset)
	}
	state := LumaUpdateState()
	if state["phase"] != "found" || state["latest_version"] != "2.1.15" {
		t.Fatalf("update state = %#v, want found 2.1.15", state)
	}
}
...
func TestLumaUpdateCheckTreatsLatest214AsCurrent(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]any{
			...
			"prerelease": false,
			"assets": []map[string]any{
				{"name": "LumaForge-2.1.14-desktop.zip", "browser_download_url": "https://cdn.example.com/current.zip"},
			},
		})
	}))
	defer server.Close()
	...
	result := LumaUpdateCheck()
	if result["current_version"] != "2.1.14" {
		t.Fatalf("current version = %#v, want 2.1.14", result["current_version"])
	}
	if result["latest_version"] != "2.1.14" || result["is_newer"] != false || result["selected_asset"] != nil {
		t.Fatalf("2.1.14 should be treated as current, got %#v", result)
	}
	state := LumaUpdateState()
	if state["phase"] != "idle" {
...
func TestReleaseVersionAndVersionComparisonHelpers(t *testing.T) {
	cases := []struct {
		...
		{map[string]any{"tag_name": "", "name": "V2.1.14"}, "2.1.14"},
